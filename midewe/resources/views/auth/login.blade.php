...
<body class="align">
    ...
    @if (session('status'))
    <ul>
        <li class="text-White"> {{ session('status') }}</li>
    </ul>
    @endif
    <div class="grid">
        <form action="{{ route('HandleLogin') }}" method="post" class="form login">
            {{ csrf_field() }}
            <div class="form__field">
                <label for="login__Email"><svg class="icon">
                        <use xlink:href="#icon-user"></use>
                    </svg><span class="hidden">Email</span></label>
                <input autocomplete="email" id="login__email" type="text" name="email" class="form__input" placeholder="email" >
            </div>
